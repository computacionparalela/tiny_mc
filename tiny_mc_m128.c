...
static void photon(int photons, float * _seed)
{
        ...
        const __m128 shells_per_mfp = _mm_div_ps(_mm_div_ps(_mm_set1_ps((float)1e4), _mm_set1_ps((float)MICRONS_PER_SHELL)), _mm_add_ps(_mm_set1_ps(MU_A), _mm_set1_ps(MU_S)));
        ...
        do {
                ...
                __m128 shell = _mm_add_ps(_mm_min_ps(_mm_floor_ps(_mm_mul_ps(sroot, shells_per_mfp)), _mm_sub_ps(_mm_set1_ps(SHELLS), _mm_set1_ps(1.0f))), _mm_set1_ps(0.1f));
